    ...
    $('#tabla_hproductos').DataTable( {
        ...
        buttons: [
            ...
            },
            {
				...
				title: 'Historial Entrada de producto Full Servi',
                extend: 'pdfHtml5',
                exportOptions: {
                    columns: [ 0, 1, 2, 5,6,7 ]
                }
...
<?php
	...
	$productos_consulta=mysql_query("select * from historial_productos join productos using(id_producto) join proveedores using(id_proveedor) where id_producto=$producto and tipo_operacion_hp='entrada' order by fecha_hp ");
	...
	<tbody>
	    ...
	    <td><? echo formato_fecha("normal",$datos_producto["fecha_hp"]) ?></td>
			...
			<td><? echo $datos_producto["nombre_proveedor"] ?></td>
 		    ...
 		    <td><? echo formato_numero(1,$datos_producto["costo_hp"])  ?></td>	
			...
			<td>
			 		...
			 		<button  <? if($datos_producto['estado_hp']==0){echo("disabled");} ?> type="button" class="btn btn-danger btn-xs" title="Anular"  onClick="anular_entrada(this.id)" id="<? echo($datos_producto["id_hp"]."_anular") ?>"><i class="fa fa-minus"> </i></button>
